...
(select 'Employee' as source_code,count(*) as src_counts ,salary as src_salary 
 FROM {{source('ANALYTICS','EMPLOYEES')}} group by all) src
inner join
( select * from
(select  source_code,sum(counts) as tgt_counts,trgt_salary from  
( SELECT 'Employee' AS source_code, count(*) as counts,employee_salary as trgt_salary 
 FROM {{ref('employees_table')}}
 group by all
)group by all )) tgt
